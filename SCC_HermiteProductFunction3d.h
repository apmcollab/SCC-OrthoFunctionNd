...
#ifndef  SCC_HERMITE_PRODUCT_FUNCTION_3D_
#define  SCC_HERMITE_PRODUCT_FUNCTION_3D_
//
//######################################################################
//                SCC_HermiteProductFunction3d.h
//######################################################################
//
/*

The HermiteProductFunction3d creates functions from R^3 -> R that
...
shifted to arbitrary locations (shiftX, shiftY, shiftZ) in R^3.
...
class HermiteProductFunction3d
{
    ...
    std::function< double(double,double,double) > H = [=](double x,double y, double z)
	{
    	...
    	return std::exp(val)*PX(gammaX*(x-shiftX))*PY(gammaY*(y-shiftY))*PZ(gammaZ*(z-shiftZ));
	};
